...
const Home: React.FC = () => {
  ...
  const displayTools = useMemo(() => {
    ...
    return filteredTools.filter(tool => tool.category === selectedCategory);
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
          <div className="text-center space-y-6 animate-fade-in">
            <h1 className="text-4xl sm:text-5xl md:text-6xl font-display font-bold text-dark-900 leading-tight">
              Your <span className="text-gradient">All-in-One</span> Toolkit
              <br />
              for Everyday Tasks
            </h1>
            <p className="text-xl text-dark-600 max-w-3xl mx-auto">
              Access powerful online tools for text processing, file conversion, development,
              calculations, and creative work. All free, fast, and privacy-focused.
            </p>
            
            <div className="flex flex-wrap justify-center gap-3 text-sm text-dark-600">
              <span className="flex items-center space-x-1 bg-white px-4 py-2 rounded-full shadow-sm">
                <span>✓</span>
            ...
            <div className="pt-8 animate-slide-up" style={{ animationDelay: '0.2s' }}>
              <SearchBar
                ...
                placeholder="Search for tools... (e.g., 'JSON', 'calculator', 'compress')"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          {categoryStats.map(cat => (
            <button
              ...
              className={`category-badge border-2 transition-all ${
                selectedCategory === cat.name
                  ? getCategoryColor(cat.name) + ' shadow-md scale-105'
          ...
          <>
            <div className="mb-6">
              <h2 className="text-2xl font-display font-bold text-dark-900">
                {searchQuery
                  ? `Search Results (${displayTools.length})`
      ...
      <section className="bg-gradient-to-r from-primary-500 to-orange-500 text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <p className="text-xl mb-8 opacity-90">
            Start using our tools now — no sign-up required!
          </p>
          <Link
            to="/tools"
            className="inline-block bg-white text-primary-600 px-8 py-4 rounded-xl font-semibold text-lg hover:bg-dark-50 transition-all duration-300 shadow-xl hover:shadow-2xl hover:-translate-y-1"
          >
            Browse All Tools →
          </Link>
        </div>
